  ...
  function initializeExtension() {
    ...
    function parsingData() {
      ...
      catalogItem.forEach((card, index) => {
        ...
        const originalPrice = price; // Сохраняем исходную цену
        let maxDiscount = 0;
        ...
        if (priceWhitePromocode__input.checked) {
          ...
          price = originalPrice; // Возвращаем исходную цену при снятии фильтра
        }

        items.push({
          product_name: name,
          shop_name: shop_name,
          originalPrice: originalPrice, // Сохраняем исходную цену
          originalShopName: shop_name, // Сохраняем исходное название магазина
          product_link: link,
          price: price,
          bonus: bonus_total,
          total: total,
    ...
    th_shop_fillter_link.addEventListener("click", () => {
      items.sort((a, b) => {
        const nameA = a.originalShopName.toUpperCase();
        const nameB = b.originalShopName.toUpperCase();
        if (nameA < nameB) {
          return -1;
        }
        if (nameA > nameB) {
    ...
    function updateProductCount() {
      const catalogItem = document.querySelectorAll('.catalog-item:not(.catalog-item_out-of-stock)');
      count_products.innerText = 'Количество товаров: ' + catalogItem.length;
    }
